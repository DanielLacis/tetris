(function () {
  ...
  // Refactor to eliminate calling so many different methods
  // Game.prototype.doThing = function (thing, sound) {
  //   var dupedPiece = this.piece.dup();
  //   dupedPiece[thing]();
  //   if (!this.board.collision(dupedPiece)) {
  //     if(this.piece[thing]()) {
  ...
  Game.prototype.drawNextPieces = function(nextPieceCtx) {
    ...
    for (var c = 0; c < 3; c++) {
      for(var a = 0; a < 6; a++) {
        for(var b = 0; b < 6; b++) {
          x = b * Piece.DIM + 6 * c * Piece.DIM;
          y = a * Piece.DIM;
          Util.drawRect(nextPieceCtx, x, y, Board.COLOR);
        }
      }
    }
    ...
    for(var i = 0; i < this.nextPiecesArray.length; i++) {
      for(var j = 0; j < this.nextPiecesArray[i].occupied.length; j++) {
        ...
        Util.drawPieceRect(nextPieceCtx, x, y, this.nextPiecesArray[i].color);
      }
    }
  };
