(function () {
  ...
  Board.prototype.draw = function(ctx, piece, drop) {
    ...
    for(var l = 0; l < drop.occupied.length; l++) {
      ...
      Tetris.Util.drawRect(ctx, x, y, "rgba(0, 0, 0, 0.1)");
    }
  };
  ...
  // if rows.length > 0, need to pause the game loop, color the rows and draw,
  // wait .2 seconds, delete the rows and draw, start the game loop
